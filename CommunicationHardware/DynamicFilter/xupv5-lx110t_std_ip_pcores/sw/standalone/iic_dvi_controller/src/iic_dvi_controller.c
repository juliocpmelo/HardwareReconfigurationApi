...
/*****************************************************************************/
/**
* @file iic_dvi_controller.c
*
* This file consists of a polled mode design example which uses the Xilinx
* IIC peripheral in dynamic mode and its low-level driver to access the
* DVI controller on the ML505/ML506/ML507 board.
*
* The DVI controller is at address 0x76 on the IIC_SCL_VIDEO, IIC_SDA_VIDEO
* IIC chain.
*
* @note
*
* This example re-uses code created for access to a generic IIC EEPROM.
* References to an EEPROM can be interpreted to mean the DVI controller chip. 
*
* <pre>
* </pre>
*
 ...
 */
#define EEPROM_TEST_START_ADDRESS   0xCA
...
int main (void)
{
   ...
   XUartNs550_SetLineControlReg(UART_BASEADDR, XUN_LCR_8_DATA_BITS); 
    ...
    print("IIC Dynamic mode DVI CONTROLLER access example - ");
    /*
    ...
    /* Hold Chrontel 7301C in Reset
     *	GPO Bit 31 (LSB) is wired to active low reset pin DVI_RESET_B
     */
    XIo_Out32((IIC_0_BASE_ADDRESS + 0x124),0xFFFFFFFF);
...
/******************************************************************************
* The function uses the low level driver of IIC to read from the IIC EEPROM
*
* @param    None.
*
* @return   XST_SUCCESS if successful else XST_FAILURE.
...
XStatus XIicLowLevelEepromDump()
{
    ...
    do
    {
         StatusReg = XIo_In32(IIC_0_BASE_ADDRESS + XIIC_SR_REG_OFFSET);
...
*********************/
    ...
    xil_printf("\r\nCalling DVI CONTROLLER ReadByte Routine\r\n");
    /*
					   ...
					   Xuint8 ByteCount)
{
     ...
     */
    SentByteCount = XIic_DynSend(IIC_0_BASE_ADDRESS, EEPROM_ADDRESS, WriteBuffer,
 					  ...
 					  Xuint8 ByteCount)
{
    ...
    do
    {
    	...
    	StatusReg = XIo_In8(IIC_0_BASE_ADDRESS + XIIC_SR_REG_OFFSET);
    	if(!(StatusReg & XIIC_SR_BUS_BUSY_MASK))
    	{
    		SentByteCount = XIic_DynSend(IIC_0_BASE_ADDRESS, EEPROM_ADDRESS,
     ...
     * Receive the data.
     */
    ReceivedByteCount = XIic_DynRecv(IIC_0_BASE_ADDRESS, EEPROM_ADDRESS,
    								 BufferPtr, ByteCount);
...
XStatus XIicLowLevelEepromInit()
{
    ...
    * Initialize the IIC Core.
    */
   Status = XIic_DynInit(IIC_0_BASE_ADDRESS);
