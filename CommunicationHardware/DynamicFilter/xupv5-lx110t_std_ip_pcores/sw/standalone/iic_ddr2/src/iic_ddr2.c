...
XStatus XIicLowLevelEepromDump()
{
    ...
    do
    {
			StatusReg = XIo_In32(IIC_1_BASE_ADDRESS + XIIC_SR_REG_OFFSET);
					   ...
					   Xuint8 ByteCount)
{
     ...
     */
    SentByteCount = XIic_DynSend(IIC_1_BASE_ADDRESS, EEPROM_ADDRESS, WriteBuffer,
...
Xuint8 EepromReadByte(AddressType Address, Xuint8 *BufferPtr, Xuint8 ByteCount)
{
    ...
    do
    {
    	...
    	StatusReg = XIo_In8(IIC_1_BASE_ADDRESS + XIIC_SR_REG_OFFSET);
    	if(!(StatusReg & XIIC_SR_BUS_BUSY_MASK))
    	{
    		SentByteCount = XIic_DynSend(IIC_1_BASE_ADDRESS, EEPROM_ADDRESS,
     ...
     * Receive the data.
     */
    ReceivedByteCount = XIic_DynRecv(IIC_1_BASE_ADDRESS, EEPROM_ADDRESS,
    								 BufferPtr, ByteCount);
...
XStatus XIicLowLevelEepromInit()
{
    ...
    * Initialize the IIC Core.
    */
   Status = XIic_DynInit(IIC_1_BASE_ADDRESS);
