...
* IIC interface of the clock generator chip on the ML505/ML506/ML507 board.
*
* The clock generator is at address 0x6A on the IIC_SCL_MAIN, IIC_SDA_MAIN
* IIC chain.  Jumper J14 must be removed for IIC access to the clock generator.
*
* @note
*
...
int main (void)
{
   ...
   XUartNs550_SetLineControlReg(UART_BASEADDR, XUN_LCR_8_DATA_BITS); 
    ...
    print("IIC Dynamic mode CLOCK GENERATOR access example - ");
    /*
...
XStatus XIicLowLevelEepromDump()
{
    ...
    /*
     * Skip writing to EEPROM for Read Clock Generator example. 
     */
/********************
...
*********************/
    ...
    xil_printf("\r\nCalling CLOCK GENERATOR ReadByte Routine\r\n");
    /*
     * Read from the Clock Generator IIC interface.
     */
    ...
    do
    {
    	...
    	if(!(StatusReg & XIIC_SR_BUS_BUSY_MASK))
    	{
    		SentByteCount = XIic_DynSend(IIC_BASE_ADDRESS, EEPROM_ADDRESS,
                                   (Xuint8 *)CommandBuf, sizeof(CommandBuf),
    								 XIIC_REPEATED_START);
	}
    }while(SentByteCount != sizeof(CommandBuf));
