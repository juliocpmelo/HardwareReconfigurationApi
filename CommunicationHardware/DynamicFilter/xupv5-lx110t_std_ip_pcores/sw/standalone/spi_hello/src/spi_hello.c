...
int main()
  ...
  XTft_SetPos(&Tft, 5, 0);
  XTft_WriteColorString(&Tft, "The FPGA has been configured from the SPI memory (SPI Config Mode)", 0x00FFFFFF, 0x00404040);

  xil_printf("\n\rThe FPGA has been configured from the SPI memory (SPI Config Mode)\n\r");
  //xil_printf("lcd_text = %s\n\r", lcd_text);
  xil_printf("\n\r\n\r");
  ...
  LCDPrintString (lcd_text, "Using SPI Mem");
