...
void CommunicationHardwareConverterVHDL::buildComponentDescription(std::string componentPath, HardwareComponent* communicationHardwareComponent){
	...
	if (mkdir (string(componentPath + "/commHardwareComponents").c_str(), 0755) == -1) //TODO check when the file exists
		cout <<"Failed to create directory " << componentPath + "/commHardwareComponents" << endl;
	...
	if (reg->assignedTopComponent != NULL){
		...
		for(set<string>::iterator it = swAccessibleInterfaces.begin(); it!= swAccessibleInterfaces.end(); it ++){
			...
			if(swAccessibleInterface->type->size() > 1)
				outFile <<"\t"<<"\t"<<swAccessibleInterface->name + "_reg <= registerInValue("<<swAccessibleInterface->type->size()-1<<" downto 0);"<<endl;
			else
				outFile <<"\t"<<"\t"<<swAccessibleInterface->name + "_reg <= registerInValue(0);"<<endl;
